...
#	Date:		2011-08-14
#
#	Purpose: 	Process the current file with wkhtmltopdf
#
#	URL:
...
####|####|####|####|####|####|####|####|####|####|####|####|####|####|####
# change `wkhtmltopdf` to whatever the command line tool is that you will be using
if ((! $+commands[wkhtmltopdf] ))
then

	# note: if desired command is a function or alias it will come back not found

	echo "	$NAME Fatal error: wkhtmltopdf --encoding 'utf-8'  is required but not found in $PATH"
	exit 1
...
if [ "$EXIT" = "0" ]
then
		# success! Assuming that 'wkhtmltopdf' (or whatever you use) is smart enough to use exit codes properly.
		...
		echo "$NAME: wkhtmltopdf failed (\$EXIT = $EXIT)"
