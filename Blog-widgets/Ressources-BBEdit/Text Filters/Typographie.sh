...
#sed -e "s/\"\([^\"]*\)\"/$go$fin\1$fin$gf/g"
#sed -e "s/$del//g" -e "s/$des1/<script/g" -e "s/$des2/script>/g" | sed -e "s/>[^<]*</$del&$del/g" | sed -e "s/\([\?\!\:\;]\)/$dec\1$dec/g" -e ":a" -e "s/\($del>[^$del]*\)\([^\($esp$nbsp$fin\)]\)[$esp$nbsp$fin]*$dec\([\?\!\:\;]\)$dec[$esp$nbsp$fin]*/\1\2$fin\3$esp/" -e "ta" -e "s/$dec//g"
