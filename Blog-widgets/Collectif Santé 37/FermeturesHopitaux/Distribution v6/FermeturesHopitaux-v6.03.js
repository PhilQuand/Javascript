...
$.fn.mapAllBlogs = function() {
  ...
  function runMapandTable(options) {
    ...
    function runData(indexEvent, isToSaveData) {
      ...
      var menuLegend = -1;
      var indexEventTable, myTable, initSearchTable, isSearchBuilder = false;
...
$('.dt-button.searchBT').on('click', function() {
    ...
    isSearchBuilder = true;
      ...
      function updateTableFilterOn() {
        var filerRows = myTable.rows({
          search: 'applied'
        }).data();
        if (indexEventTable.length !== filerRows.length) {
          tableFilterOn = true;
          for (var i = 0; i < indexEventTable.length; i++) indexEventTable[i]["selected"] = false;
