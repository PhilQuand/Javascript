  ...
  $.fn.InfosComités = function(options) {
    ...
    Promise.all([
      ...
      Promise.all([
        ...
        var infoRows = $(_this).find('.infoEvents-wrapper');
        if (infoRows.length == 0) {
          var infoRows = $('<div class="infoEvents-wrapper"></div>');
          $(_this).append(infoRows);
        }
        ...
        function getMyInnerLinkContent() {
          ...
          function isLinkContent(weekInfos) {
            ...
            $('.infoEvents-wrapper').find('.inforWeek').each(function() {
              if (weekInfos == '' || weekInfos == $(_this).attr('data-weekInfos')) {
                var result = $(this).find('.inforEvent');
                var itemsLength = result.length
                if (itemsLength > 0) {
                  result.each(function() {
                    var infoPopUps = JSON.parse($(_this).find('.markerLocation').html());
                    ...
                    infoPopUps.doc = $(_this).find('.markerPopUp').html()
          ...
          function getBody(content) {
            ...
            if (pictures.length > 0) {
              ...
              pictures.each(function() {
                var textPicture = $(_this).prev().text().split(" ");
                if (textPicture == "") textPicture = $(_this).parent().prev().text().split(" ");
                textPicture = textPicture[textPicture.length - 1];
                hrefImg = $(_this).attr('src');
        ...
        if (hrefLength > 1) {
          ...
          var dateFormat = "yy-mm-dd",
            ...
            .on("change", function() {
              to.datepicker("option", "minDate", getDate(_this));
              ...
              else var firstDate = new Date(moment(getDate(_this), 'yy-mm-dd').day(-6));
            ...
            .on("change", function() {
              from.datepicker("option", "maxDate", getDate(_this));
              ...
              else var lastDate = new Date(moment(getDate(_this), 'yy-mm-dd').day(0));
